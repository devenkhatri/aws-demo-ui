function adaptFilterType(type) {
  ...
  export function adaptFacetConfig(facets) {
    ...
    const convertInvalidFacetsToUndefined = ([fieldName, config]) => {
      if (config.type != "value") {
        console.warn(
          `search-api-connector: Dropping ${fieldName} facet, only value facets are supported in Site Search`
        );
        return;
      }
      if (config.sort) {
        console.warn(
          "search-api-connector: Site Search does not support 'sort' on facets"
        );
      }
      if (config.size) {
        console.warn(
          "search-spi-connector: Site Search does not support 'size' on facets"
        );
      }
  ...
  export function adaptFilterConfig(filters) {
    ...
    return filters.reduce((acc, filter) => {
      ...
      if (acc[fieldName]) {
        console.warn(
          "search-api-connector: More than one filter found for a single field"
        );
        return acc;
      }
      ...
      if (filter.type && (filter.type !== "all" && filter.type !== "any")) {
        console.warn(
          `search-api-connector: Unsupported filter type "${filter.type}" found, only "all" and "any" are currently supported`
        );
        return acc;
      }
      ...
      if (fieldValue.find(v => typeof v === "object") !== undefined) {
        if (fieldValue.length > 1) {
          console.warn(
            "search-api-connector: Cannot apply more than 1 none-value filters to a single field"
          );
          return acc;
        }
